...
from diffusers.schedulers import DDIMScheduler, DDPMScheduler  # noqa: F401
from huggingface_hub import upload_folder
from tqdm.auto import tqdm

from models.bcd_pipeline import BodyCorrectionPipelineWithoutVAE, encode_text
from tools.init_1 import initialize
from tools.loss import ReconLoss, calc_texture_loss, get_pred_img
from tools.utils_1 import log_validation_dm, read_data, save_model_card, test_vae, unwrap_model # noqa: F401


def main():
    # =========== initialize ===========
    cfg, logger, accelerator, _, \
            ...
            with accelerator.accumulate(unet, text_encoder):
                ...
                if cfg.train.noise_offset:
                    ...
                    noise += cfg.train.noise_offset * torch.randn(
                        (inputs.shape[0], inputs.shape[1], 1, 1), device=inputs.device
                    )
                ...
                if cfg.train.input_perturbation:
                    noisy_inputs = noise_scheduler.add_noise(inputs, new_noise, timesteps)
                else:
                ...
                unet_inputs = torch.cat([noisy_inputs, condition], dim=1)
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    target = noise_scheduler.get_velocity(inputs, noise, timesteps)
                ...
                if cfg.train.snr_gamma is None:
                    ...
                    mse_loss_weights = torch.stack([snr, cfg.train.snr_gamma * torch.ones_like(timesteps)], dim=1).min(
                        dim=1
                    )[0]
                    if noise_scheduler.config.prediction_type == "epsilon":
                ...
                if cfg.loss.use_recon_loss or cfg.loss.use_texture_loss or cfg.loss.use_fidelity_loss:
                    pred_imgs = get_pred_img(cfg, model_pred, noise_scheduler, vae, bsz, timesteps, noisy_inputs)
                
                if cfg.loss.use_fidelity_loss:
                    fidelity_loss = ReconLoss()(pred_imgs, gt_imgs, coeff=cfg.loss.fidelity_loss_weight)
                    loss += fidelity_loss

                # Texture loss
                if cfg.loss.use_texture_loss:
                    texture_loss = calc_texture_loss(cfg, pred_imgs, gt_imgs)
                    loss += texture_loss
