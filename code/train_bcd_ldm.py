...
from tools.loss import calc_recon_loss, calc_texture_loss, get_pred_img

from models.bcd_pipeline import BodyCorrectionInpaintingPipeline, encode_empty_text
...
def main():
    ...
    unet, ema_unet, vae, text_encoder, \
    ...
    for epoch in range(first_epoch, cfg.train.epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, text_encoder):
                ...
                downscale_factor = input_imgs.shape[-1] // latent_inputs.shape[-1]
                # conditions 
                ...
                # hole mask imgs downsample
                # down_hole_mask = F.interpolate(hole_masks, scale_factor=1/downscale_factor)
                height, width = hole_masks.shape[-2:]
                hole_masks_downscaled = torch.nn.functional.interpolate(hole_masks, size=(height // downscale_factor, width // downscale_factor))

                # Sample noise that we'll add to the latents
                noise = torch.randn_like(latent_inputs)

                # # only hole region has noise
                # noise = noise * hole_mask_downscaled
                ...
                # noisy_latents = down_hole_mask_imgs * noisy_latents + (1 - down_hole_mask_imgs) * latent_inputs
                
                # Concatenate noisy latents and conditionings to get inputs to unet
                unet_inputs = torch.cat([noisy_latents, hole_masks_downscaled, latent_img_with_holes], dim=1)

                # Get the text embedding for conditioning
                empty_text_embed = encode_empty_text(tokenizer, text_encoder).to(weight_dtype)
                empty_text_embed = empty_text_embed.repeat(bsz, 1, 1)
                ...
                model_pred = unet(unet_inputs, timesteps, empty_text_embed, return_dict=False)[0]
